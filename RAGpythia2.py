...
class PythiaRAG:
    def __init__(self, db_path="document_vectors.db", faiss_path="docs.faiss"):
        ...
        # Initialize Pythia model
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.tokenizer = AutoTokenizer.from_pretrained("EleutherAI/pythia-1.4b")
        self.model = AutoModelForCausalLM.from_pretrained("EleutherAI/pythia-1.4b").to(self.device)
    ...
    def generate(self, query: str, context: str) -> str:
        """Generate answer using Pythia-1.4b"""
        try:
            prompt = f"""You are a scientific assistant. Answer the user's question using ONLY the context below.
...
Answer:"""
            ...
            # Generate with parameters to reduce repetition
            ...
            # Clean up any trailing incomplete sentences or repetitions
            if "." in answer:
            ...
            print(f"Pythia generation failed: {str(e)}")
...
df_results.to_csv("evaluation_results_pythia.csv", index=False)
